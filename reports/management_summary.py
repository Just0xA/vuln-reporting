...
def compute_all_metrics(
    ...
    # DEPRECATED: _compute_metric_1() kept for reference — now delegates to module
    _m1_data = _module_registry.get("total_vulns_by_severity")().compute(
        vulns_df, assets_df, report_date,
        _ModuleConfig("total_vulns_by_severity"),
    )
    m1 = {k: _m1_data.metrics[k] for k in ("critical", "high", "medium", "low", "total")}
    ...
    # DEPRECATED: _compute_metric_3() kept for reference — now delegates to module
    _m3_data = _module_registry.get("mttr_by_severity")().compute(
        fixed_vulns_df, assets_df, report_date,
        _ModuleConfig("mttr_by_severity"),
    )
    _mttr_status_colors = {
        "Within SLA":     "#388e3c",
        "Near SLA Limit": "#fbc02d",
        "Exceeding SLA":  "#d32f2f",
    }
    m3 = {
        "mttr": {
            sev: _m3_data.metrics.get(f"{sev}_mttr")
            for sev in SLA_DAYS
        },
        "status": {
            sev: (
                None
                if _m3_data.metrics.get(f"{sev}_status") == "No Data"
                else _m3_data.metrics.get(f"{sev}_status")
            )
            for sev in SLA_DAYS
        },
        "color": {
            sev: _mttr_status_colors.get(
                _m3_data.metrics.get(f"{sev}_status", ""), "#9E9E9E"
            )
            for sev in SLA_DAYS
        },
        "total_fixed": _m3_data.metrics.get("sample_size", 0),
    }

    logger.info("[%s] Computing metric 4 — Patch Compliance Rate", REPORT_SLUG)
    # DEPRECATED: _compute_metric_4() kept for reference — now delegates to module
    _m4_data = _module_registry.get("patch_compliance_rate")().compute(
        vulns_df, assets_df, report_date,
        _ModuleConfig("patch_compliance_rate"),
    )
    _m4_raw_rate = _m4_data.metrics.get("overall_rate")
    # Preserve legacy behaviour: overall_rate = 100.0 when total_open == 0
    _m4_overall  = _m4_raw_rate if _m4_raw_rate is not None else 100.0
    if _m4_overall >= 90:
        _m4_status, _m4_color = "Compliant",       "#388e3c"
    elif _m4_overall >= 75:
        _m4_status, _m4_color = "Needs Attention",  "#fbc02d"
    else:
        _m4_status, _m4_color = "Non-Compliant",    "#d32f2f"
    _m4_per_sev = {
        row["severity"].lower(): {
        ...
        for row in _m4_data.table_data
    }
    m4 = {
        "overall_rate": _m4_overall,
        "per_severity": _m4_per_sev,
        "within_sla":   _m4_data.metrics.get("within_sla", 0),
        "total_open":   _m4_data.metrics.get("total_open", 0),
        "status":       _m4_status,
        "color":        _m4_color,
    }
...
# draw_gauge() is imported from reports.modules.chart_utils
# ===========================================================================
