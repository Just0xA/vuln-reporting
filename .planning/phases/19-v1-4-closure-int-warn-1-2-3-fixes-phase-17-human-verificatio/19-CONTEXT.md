...
5. **CodeRabbit pass** — ✅ run + triaged (2026-06-24). 41 findings → 39 net-new (`CR-*`) inventoried in `19-CODERABBIT-TRIAGE.md`; all in fix-all scope per D-02. Planner consumes the triage, not the raw stream.
...
- `.planning/phases/19-v1-4-closure-int-warn-1-2-3-fixes-phase-17-human-verificatio/19-CODERABBIT.md` — **produced** (2026-06-24, `coderabbit review --agent --base-commit f3f1c4f`, free CLI tier): raw JSONL findings stream, 41 findings (1 critical, 22 major, 18 minor).
- `.planning/phases/19-v1-4-closure-int-warn-1-2-3-fixes-phase-17-human-verificatio/19-CODERABBIT-TRIAGE.md` — **the planner's input.** Deduplicated triage of the raw stream: 2 exact duplicates of existing review findings, 1 partial overlap (extends WR-07), **39 net-new findings** assigned `CR-*` IDs and grouped into wave-ready buckets (A security/fail-closed ×5 incl. the lone critical CR-C1; B trend/backfill correctness ×7; C fail-soft/render ×4; D module copy/audit ×2; E test rigor ×7; F deploy script ×2; G doc/comment cleanup ×12). All 39 are in fix-all scope per D-02.
