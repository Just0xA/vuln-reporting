...
- **🔴 v1.2 ship-blocker — systemd venv path mismatch (found 2026-05-20 during Phase 11 planning):** `deploy/vuln-reports.service:42` has `ExecStart=/opt/vuln-reporting/.venv/bin/python scheduler.py --mode daemon`, but Phase 10's `update_from_github.sh` (`provision_venv`, line 408) builds the venv **per-release** at `releases/vX.Y.Z/.venv` (reached via `current/.venv`). After a tarball install there is no `/opt/vuln-reporting/.venv`, so the unit fails to start — and the updater's post-swap `systemctl is-active` health check would then auto-rollback every upgrade. **Fix:** change `ExecStart` (and any sibling path) to `/opt/vuln-reporting/current/.venv/bin/python` (matches `WorkingDirectory=/opt/vuln-reporting/current/`). Must land before tagging v1.2. Best handled as a `/gsd-quick` task against `deploy/vuln-reports.service` — touches a Phase-7 artifact, so out of scope for the Phase 11 docs work where it was discovered.
