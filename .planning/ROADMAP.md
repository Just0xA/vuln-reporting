...
Substrates:
- **S1 — Trend-reconstruction engine (KEYSTONE)** — reconstruct historical open-population from `first_found`/`last_fixed`/`state`/`severity` instead of snapshots; powers ~6 of the batch reports, retires snapshot `data/trend/`. Caveats: as-of-date severity drift, retention/lookback window. Design: [`notes/trend-reconstruction-engine.md`](notes/trend-reconstruction-engine.md).
