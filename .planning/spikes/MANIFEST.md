...
| 003 | was-access-path | standard | Whether WAS findings come via `tio.exports.vulns()` (light) or a separate export (heavy), and which web fields exist — prerequisite for the External/DMZ report (#3) | ⚠ PARTIAL — RESOLVED by docs: WAS is a SEPARATE export (`tio.exports.was()`), so HEAVY (new fetcher). Tenant probe (`probe_live.py`) still needed for licensing + whether VPR is populated on WAS | was, external, dmz, fetcher, research |
