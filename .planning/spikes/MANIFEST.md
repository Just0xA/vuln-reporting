...
| 003 | was-access-path | standard | Whether WAS findings come via the vuln export (light) or a separate export (heavy), and which web fields exist — prerequisite for the External/DMZ report (#3) | ⚠ PARTIAL — SEPARATE export (HEAVY). Live probe showed pinned **pyTenable 1.5.2 has NO `tio.exports.was()`**; only scan-config-driven `tio.was.export()` (undocumented endpoint, per-scan downloads, client-side filtering). New decision: upgrade SDK vs build on `tio.was`. Corrected probe still needed for licensing + VPR-on-WAS | was, external, dmz, fetcher, research, sdk-version |
