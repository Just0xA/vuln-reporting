...
| 003 | was-access-path | standard | Whether WAS findings come via the vuln export (light) or a separate export (heavy), and which web fields exist — prerequisite for the External/DMZ report (#3) | ✓ VALIDATED w/ constraints — WAS **licensed & reachable**; SEPARATE export (HEAVY). Pinned **1.5.2 `tio.was`** = per-scan snapshots, **no VPR** (severity = `risk_factor`/CVSS), **no lifecycle fields** (no trend/SLA). Trend/SLA needs a **pyTenable upgrade** to `tio.exports.was()` (vs SDK-version constraint). Schema captured | was, external, dmz, fetcher, sdk-version, risk-factor |
