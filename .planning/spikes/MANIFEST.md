...
| 003 | was-access-path | standard | Given the tenant's WAS license, when web-app findings are fetched, then determine whether they arrive via `tio.exports.vulns()` (unified) or require `tio.was`, and which web fields (URL/OWASP/CWE/method) are available — prerequisite for the External/DMZ report (#3) | 📋 PROPOSED (needs live API or doc research; cache can't answer it) | was, external, dmz, fetcher, research |
